...
export const Header = ({ setMobileOpen }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 w-full glass-panel border-b border-white/20 bg-slate-950/20 dark:bg-slate-950/25 backdrop-blur-3xl px-4 sm:px-8 py-4 transition-colors">
        <div className="flex items-center justify-between gap-4">
          {/* Left: Mobile Toggle & Page Header Title */}
          <div className="flex items-center gap-3">
            <button
              onClick={() => setMobileOpen(true)}
              className="lg:hidden p-2 rounded-xl text-slate-200 hover:text-white hover:bg-white/10"
            >
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            {/* Glass Command Palette Trigger Button */}
            <button
              onClick={() => setShowCommandPalette(true)}
              className="hidden md:flex items-center justify-between gap-3 px-3.5 py-2 w-48 xl:w-64 text-xs rounded-xl bg-black/30 dark:bg-black/40 border border-white/20 text-slate-200 hover:text-white hover:bg-white/15 backdrop-blur-xl transition-all text-left shadow-inner"
            >
              <div className="flex items-center gap-2">
                <Search className="w-4 h-4 text-brand-300" />
                <span className="truncate font-medium">Search or commands...</span>
              </div>
              <kbd className="flex items-center gap-0.5 px-2 py-0.5 rounded-lg bg-white/15 text-[10px] font-extrabold text-white border border-white/10">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3.5 py-2 text-xs font-bold rounded-xl bg-black/30 dark:bg-black/40 border border-white/20 text-white hover:bg-white/15 backdrop-blur-xl transition-all shadow-inner"
              >
              ...
              <AnimatePresence>
                {showDatePicker && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-44 glass-panel rounded-2xl p-2 shadow-2xl border border-white/25 bg-slate-950/70 backdrop-blur-3xl z-50 text-xs text-white"
                  >
                    {[
            ...
            <button
              onClick={toggleTheme}
              className="p-2.5 rounded-xl bg-black/30 dark:bg-black/40 border border-white/20 text-slate-200 hover:bg-white/15 backdrop-blur-xl transition-all shadow-inner"
            ...
            <div className="relative">
              <button
                ...
                className="relative p-2.5 rounded-xl bg-black/30 dark:bg-black/40 border border-white/20 text-slate-200 hover:bg-white/15 backdrop-blur-xl transition-all shadow-inner"
              >
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    className="absolute right-0 mt-2 w-80 sm:w-96 glass-panel rounded-2xl p-4 shadow-2xl border border-white/25 bg-slate-950/70 backdrop-blur-3xl z-50 text-xs text-white"
                  >
                    <div className="flex items-center justify-between pb-3 border-b border-white/15 mb-3">
                      ...
                      <span className="text-[10px] bg-brand-500/35 text-brand-300 font-bold px-2 py-0.5 rounded-full border border-brand-500/30">
                    ...
                    <div className="space-y-2.5 max-h-72 overflow-y-auto no-scrollbar">
                      {notifications.map((n) => (
                        <div
                          key={n.id}
                          className="p-3 rounded-xl bg-white/10 border border-white/15 flex gap-3 items-start backdrop-blur-xl"
                        >
                          {n.type === 'warning' ? (
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 p-1.5 pr-3 rounded-xl bg-black/30 dark:bg-black/40 border border-white/20 hover:bg-white/15 backdrop-blur-xl transition-all shadow-inner"
              >
                <img
                  src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&q=80"
                  alt="Admin Avatar"
                  className="w-7 h-7 rounded-lg object-cover ring-2 ring-brand-400/50"
                />
              ...
              <AnimatePresence>
                {showProfileMenu && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    className="absolute right-0 mt-2 w-52 glass-panel rounded-2xl p-2 shadow-2xl border border-white/25 bg-slate-950/70 backdrop-blur-3xl z-50 text-xs text-white"
                  >
                    ...
                    <button className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-slate-100 hover:bg-white/15 transition-colors font-medium">
                      <User className="w-4 h-4 text-brand-300" /> Profile Settings
                    </button>
                    <button className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-slate-100 hover:bg-white/15 transition-colors font-medium">
